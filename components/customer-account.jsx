...
function Tracking({ order }) {
  ...
  const events = [...(order.tracking?.events || [])].reverse();
  return <article className={`customer-tracking ${order.status === "Delivery exception" ? "has-exception" : ""}`}>
    <header><div><span>{order.id}</span><h3>{order.items.map((item) => item.name).join(", ")}</h3></div><b>{order.tracking?.eta && order.tracking.eta !== "—" ? `Est. ${new Date(order.tracking.eta).toLocaleDateString()}` : "Preparing details"}</b></header>
    {order.tracking?.exception && <div className="tracking-exception"><b>Delivery exception</b><span>{order.tracking.exception}</span></div>}
    <div className="customer-tracking-steps">{steps.map((step,index) => <span className={index + 1 <= (order.tracking?.step || 0) ? "done" : ""} key={step}><i>{index + 1 < (order.tracking?.step || 0) ? "✓" : index + 1}</i><b>{step}</b></span>)}</div>
    <footer><span>{order.tracking?.carrier}{order.tracking?.service ? ` · ${order.tracking.service}` : ""}</span><b>{order.tracking?.number}</b>{order.tracking?.number !== "—" && <button onClick={() => navigator.clipboard?.writeText(order.tracking.number)}>Copy tracking</button>}</footer>
    <div className="customer-shipment-facts"><span><small>Packages</small><b>{order.tracking?.packageCount || 1}</b></span><span><small>Weight</small><b>{order.tracking?.weight || "Pending"}</b></span><span><small>Signature</small><b>{order.tracking?.signatureRequired ? "Required" : "Not required"}</b></span><span><small>Insurance</small><b>{order.tracking?.insuredValue ? `$${Number(order.tracking.insuredValue).toLocaleString()}` : "Pending"}</b></span></div>
    {events.length > 0 && <section className="customer-event-history"><h4>Shipment updates</h4>{events.map((event) => <div key={event.id}><i /><span><b>{event.status}</b><small>{event.location} · {new Date(event.date).toLocaleString()}</small>{event.note && <p>{event.note}</p>}</span></div>)}</section>}
  </article>;
}
