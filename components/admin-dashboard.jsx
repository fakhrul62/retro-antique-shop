...
function OrderRecord({ order, update, back, flash }) {
  ...
  const packingFields = [["conditionVerified","Condition matched report"],["accessoriesChecked","Keys and accessories checked"],["protectiveWrap","Museum-safe wrap complete"],["crateSealed","Crate sealed and photographed"],["insured","Shipping insurance confirmed"]];
  const packingComplete = packingFields.every(([field]) => order.fulfillment?.[field]);
  ...
  return <><button className="back-button" onClick={back}>← Back to orders</button><SectionHead eyebrow={`${order.id} · ${fmtDate(order.date)}`} title={order.customer.name} copy={`${order.customer.email} · ${order.customer.phone || "No phone"}`} action={<Status value={order.status} />} />
    <div className="tracking-rail">{steps.map((step, index) => <button className={index + 1 <= (order.tracking?.step || 0) ? "done" : ""} key={step} onClick={() => setStage(step)}><i>{index + 1 < (order.tracking?.step || 0) ? "✓" : index + 1}</i><b>{step}</b></button>)}</div>
    ...
    <section className="dash-card wide packing-card"><CardTitle title="Packing checklist" meta={`${packingFields.filter(([field]) => order.fulfillment?.[field]).length}/${packingFields.length} complete`} /><div className="packing-grid">{packingFields.map(([field,label]) => <label key={field}><input type="checkbox" checked={Boolean(order.fulfillment?.[field])} onChange={(event) => update(order.id, { fulfillment: { [field]: event.target.checked } })} /><span><b>{label}</b><small>{order.fulfillment?.[field] ? "Completed" : "Required before packing"}</small></span></label>)}</div><div className="packer-row"><label>Packed by<input value={order.fulfillment?.packedBy || ""} onChange={(event) => update(order.id, { fulfillment: { packedBy: event.target.value } })} placeholder="Staff name" /></label><button className="primary-admin" disabled={!packingComplete} onClick={() => setStage("Packed")}>{order.status === "Packed" ? "Packed ✓" : "Mark order packed"}</button>{order.fulfillment?.packedAt && <span>Packed {new Date(order.fulfillment.packedAt).toLocaleString()}</span>}</div></section>
    ...
    <section className="dash-card"><CardTitle title="Delivery" /><Details rows={[["Carrier", order.tracking?.carrier],["Tracking", order.tracking?.number],["ETA", order.tracking?.eta]]} /><button className="secondary-admin full" onClick={() => setStage(order.status === "Packed" ? "In transit" : order.status === "In transit" ? "Delivered" : "Packed")}>{order.status === "Packed" ? "Mark in transit" : order.status === "In transit" ? "Mark delivered" : "Complete packing"}</button></section>
